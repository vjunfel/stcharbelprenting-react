...
const MobileMenu: React.FC<MobileMenuProps> = ({ setIsOpen }) => {
	...
	return (
		<div className=" fixed top-0 right-0 bottom-0 flex flex-col justify-start bg-gray-100 min-w-64 max-w-[500px] min-h-screen shadow z-100">
			<div className="w-full flex items-center justify-end px-5 py-10">
				<X
					onClick={() => setIsOpen((active) => !active)}
					className=" w-6 h-6 cursor-pointer items-end justify-end text-gray-700 active:bg-blue-100 rounded-full"
				/>
			</div>
